  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h2 className="font-display font-extrabold text-3xl text-white tracking-tight">Assignment Management</h2>
          <p className="text-slate-400 text-sm mt-1">Track deadlines, subjects, priorities, and difficulty settings.</p>
        </div>
        <button
          onClick={onAddClick}
          className="flex items-center gap-2 px-4.5 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition shadow-lg shadow-indigo-500/20 cursor-pointer"
        >
      ...
      {}
      <div className="p-4 rounded-2xl bg-slate-900 border border-slate-800 flex flex-col xl:flex-row gap-4 items-center justify-between">
        <div className="flex flex-wrap items-center gap-3 w-full xl:w-auto">
          ...
          <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800 w-full sm:w-auto">
            ...
            <span className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-wider hidden sm:inline">Sort:</span>
            <select
              value={sortBy}
          ...
          <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800 w-full sm:w-auto">
            ...
            <span className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-wider hidden sm:inline">Course:</span>
            <select
              value={courseFilter}
          ...
          <div className="flex items-center gap-2 bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-800 w-full sm:w-auto">
            <Filter className="w-3.5 h-3.5 text-slate-500 shrink-0" />
            <span className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-wider hidden sm:inline">Priority:</span>
            <select
        ...
        <div className="flex bg-slate-950 p-1.5 rounded-xl border border-slate-800 w-full xl:w-auto justify-between sm:justify-start gap-1">
          {(["ALL", "PENDING", "COMPLETED"] as const).map((status) => (
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-[10px] font-mono font-bold tracking-wider uppercase transition cursor-pointer ${
                filter === status
      ...
      {filteredAndSortedAssignments.length === 0 ? (
        <div className="py-16 text-center border border-dashed border-slate-800 rounded-2xl bg-slate-900/10">
          ...
          <p className="text-slate-400 text-sm font-semibold font-display">No assignments found</p>
            ...
            {filteredAndSortedAssignments.map((assignment) => {
              ...
              return (
                <motion.div 
                  ...
                  key={assignment.id}
                  className={`bg-slate-900/60 border rounded-2xl shadow-xl transition-colors duration-300 flex flex-col justify-between relative overflow-hidden backdrop-blur-sm group ${
                    isCompleted 
                      ? "border-slate-800/60 opacity-80 hover:opacity-100" 
                      : isOverdue
                        ? "border-rose-500/50 hover:border-rose-500 shadow-lg shadow-rose-950/25"
                        : isDueToday
                ...
                <div className="p-5.5 space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    {}
                    <span className="text-[10px] font-mono font-bold text-slate-400 bg-slate-950 px-2.5 py-1 rounded-lg border border-slate-800 max-w-[140px] truncate uppercase">
                      {assignment.course}
                    </span>

                    <div className="flex items-center gap-1.5">
                      {isOverdue && (
                        <span className="text-[9px] font-mono font-extrabold text-rose-400 bg-rose-500/15 border border-rose-500/30 px-2 py-0.5 rounded-md animate-pulse shrink-0">
                          ⚠️ OVERDUE
                        </span>
                      )}
                      {isDueToday && (
                        <span className="text-[9px] font-mono font-extrabold text-amber-400 bg-amber-500/15 border border-amber-500/30 px-2 py-0.5 rounded-md shrink-0">
                          📅 DUE TODAY
                        </span>
                      )}
                      {}
                      <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded-md border shrink-0 ${getPriorityStyles(assignment.priority)}`}>
                        {assignment.priority}
                      </span>
                      {}
                      <span className={`text-[9px] font-mono font-bold px-2 py-0.5 rounded-md border ${getDifficultyStyles(assignment.difficulty)}`}>
                  ...
                  <div className="space-y-1.5">
                    <h3 className={`text-base font-bold font-display leading-snug transition-colors ${
                      isCompleted ? "text-slate-500 line-through" : "text-white group-hover:text-indigo-300"
                    }`}>
                      {assignment.title}
                    </h3>
                  ...
                  {}
                  <div className="grid grid-cols-2 gap-3.5 py-3 border-y border-slate-800/60 text-xs">
                    {}
                    <div className="flex items-center gap-2 text-slate-400">
                      <Clock className="w-4 h-4 text-slate-500" />
                      <div>
                        <div className="text-[9px] text-slate-500 uppercase font-mono font-semibold">EST. HOURS</div>
                        <div className="text-white font-mono font-bold mt-0.5">{assignment.estimatedHours}h</div>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-slate-400 justify-end text-right">
                      <div>
                        <div className="text-[9px] text-slate-500 uppercase font-mono font-semibold">DEADLINE</div>
                      ...
                      <Calendar className={`w-4 h-4 ${isCompleted ? "text-slate-600" : isOverdue ? "text-rose-400 animate-pulse" : isDueToday ? "text-amber-400" : "text-indigo-400"}`} />
                    </div>
                  </div>
                ...
                {}
                {isExpanded && (
                  <div className="px-5.5 pb-5 pt-1 border-t border-slate-800 bg-slate-950/30 space-y-3.5 animate-fade-in">
                    <div className="space-y-2">
                      <span className="text-[9px] font-mono font-bold text-slate-500 uppercase tracking-widest block">CHECKLIST</span>
                    ...
                    <button
                      ...
                      className="w-full py-1.5 rounded-lg bg-indigo-500/5 hover:bg-indigo-500/10 border border-indigo-500/15 text-[10px] font-bold text-indigo-400 flex items-center justify-center gap-1.5 transition cursor-pointer"
                    >
                ...
                {}
                <div className="p-4 bg-slate-950/40 border-t border-slate-800/50 flex items-center justify-between gap-2 shrink-0">
                  {}
                  {isCompleted ? (
                    <button
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-transparent border border-slate-800 text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl text-xs font-semibold transition cursor-pointer"
                    >
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {!isCompleted && (
                      <button
                        ...
                        className="p-1.5 bg-slate-950 border border-slate-800 text-slate-500 hover:text-indigo-400 hover:border-indigo-500/20 rounded-lg transition cursor-pointer"
                    ...
                    <button
                      ...
                      className="p-1.5 bg-slate-950 border border-slate-800 text-slate-500 hover:text-white hover:border-slate-700 rounded-lg transition cursor-pointer"
