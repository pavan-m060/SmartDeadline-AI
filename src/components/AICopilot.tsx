...
export default function AICopilot({ assignments, studySessions }: AICopilotProps) {
  ...
  const handleAddToCalendar = () => {
    ...
    let icsContent = "BEGIN:VCALENDAR\nVERSION:2.0\nPRODID:-//Smart Deadline AI//Academic Co-Pilot//EN\n";
  ...
  return (
    ...
    >
      {}
      <div className="w-full lg:w-5/12 bg-slate-900 border border-slate-800/50 rounded-xl flex flex-col h-full overflow-hidden shadow-sm backdrop-">
        
        {}
        <div className="p-4 border-b border-slate-800/50 bg-slate-900 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-brand-purple/10 border border-indigo-500/20 flex items-center justify-center">
              <Brain className="w-5 h-5 text-slate-300 " />
            </div>
            <div>
              <h2 className="font-sans font-semibold text-sm text-slate-100">Co-Pilot Consultation</h2>
              <p className="text-xs text-slate-400 font-mono">Real-Time Cognitive Tutor</p>
            </div>
          </div>
          {messages.length > 1 && (
            <button 
          ...
          <AnimatePresence initial={false}>
            {messages.map((m) => (
              ...
              >
                ...
                <div 
                  className={`max-w-[85%] p-3.5 rounded-xl border text-sm shadow-md leading-relaxed ${
                    m.role === "user"
                      ? "bg-indigo-600 border-indigo-500 text-slate-100 rounded-br-none"
                ...
                >
                  ...
                  {m.role === "assistant" && m.plan && (
                    <div className="mt-3 pt-2.5 border-t border-slate-800 flex items-center justify-between">
                      <span className="text-xs font-mono text-emerald-400 flex items-center gap-1 bg-emerald-500/10 px-2 py-0.5 rounded-full border border-emerald-500/10">
                        <Activity className="w-3 h-3 " /> Strategic Plan Updated
                      </span>
                      <button 
                        onClick={() => {
                        ...
                        className="text-xs text-slate-300 hover:text-indigo-300 flex items-center gap-0.5 cursor-pointer"
                      >
            ...
            {isExtractingFile && (
              ...
              >
                ...
                <div className="bg-slate-900 border border-slate-800/50 max-w-[85%] p-4 rounded-xl rounded-bl-none space-y-2 flex-1">
                  <div className="flex gap-1.5 items-center">
                    <span className="text-xs text-slate-300 font-medium">Extracting syllabus details...</span>
                  </div>
            ...
            {isLoading && (
              ...
              >
                ...
                <div className="bg-slate-900 border border-slate-800/50 max-w-[85%] p-4 rounded-xl rounded-bl-none space-y-2 flex-1">
                  <div className="flex gap-1.5 items-center">
                    <span className="text-xs text-slate-300 font-medium">Co-Pilot is analyzing details...</span>
                  </div>
                  <SkeletonPulse className="w-full h-3" />
        ...
        {messages.length <= 1 && !isLoading && !isExtractingFile && (
          <div className="px-4 pb-3 pt-2 shrink-0 border-t border-slate-800 bg-slate-900">
            <span className="text-xs font-mono font-bold text-slate-500 uppercase tracking-widest block mb-2.5">Suggested Prompts</span>
            <div className="flex flex-col gap-2">
              {suggestions.map((s, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSubmit(s.text)}
                  className={`p-2.5 text-left rounded-xl bg-slate-900 border border-slate-800/50 border text-xs flex items-start gap-2.5 transition duration-200 hover:scale-[1.01] hover:brightness-110 cursor-pointer ${s.color}`}
                >
        ...
        {}
        <div className="p-4 border-t border-slate-800 bg-slate-900 flex items-center gap-2 shrink-0">
          <input
            ref={fileInputRef}
            type="file"
          ...
          <button
            ...
            className="w-11 h-11 bg-slate-850 hover:bg-slate-800 border border-slate-800 text-slate-400 hover:text-slate-100 disabled:opacity-50 rounded-xl flex items-center justify-center transition shrink-0 cursor-pointer"
          >
          ...
          <input
            ...
            disabled={isLoading || isExtractingFile}
            className="flex-1 px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-indigo-500/80 focus:ring-1 focus:ring-indigo-500/30 disabled:opacity-50"
          />
          <button
            onClick={() => handleSubmit(inputValue)}
            disabled={!inputValue.trim() || isLoading || isExtractingFile}
            className="w-11 h-11 bg-brand-purple hover:bg-brand-purple-dark shadow-sm disabled:bg-slate-800/80 disabled:text-slate-600 rounded-xl flex items-center justify-center transition shrink-0 cursor-pointer text-slate-100"
          >
      ...
      {}
      <div className="flex-1 bg-slate-900 border border-slate-800/50 rounded-xl flex flex-col h-full overflow-hidden">
        
        {}
        <div className="p-4 border-b border-slate-800/50 bg-slate-900 flex flex-wrap gap-3 items-center justify-between shrink-0">
          <div>
            <h2 className="font-sans font-semibold text-sm text-slate-100 flex items-center gap-2">
              <Compass className="w-4.5 h-4.5 text-slate-300" /> Active Strategic Plan
            </h2>
            <p className="text-xs text-slate-400 font-mono">Dynamic Academic Roadmap</p>
          </div>
          ...
          <div className="flex flex-wrap gap-2">
            {activePlan && (
              <>
                <button
                  ...
                  className="px-3 py-1.5 bg-slate-800 hover:bg-slate-750 border border-slate-700 text-slate-200 hover:text-slate-100 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer"
            ...
            <button
              ...
              className="px-3 py-1.5 bg-indigo-600/10 hover:bg-indigo-600/20 border border-indigo-500/20 hover:border-indigo-500/30 text-slate-300 hover:text-indigo-300 rounded-lg text-xs font-semibold flex items-center gap-1.5 transition cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed"
            >
        ...
        {}
        <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin bg-slate-950">
            ...
            {activePlan ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  {}
                  <div className="bg-slate-900 border border-slate-800/50 rounded-xl p-5 flex flex-col justify-between items-center text-center shadow-md relative overflow-hidden group">
                    <div className="absolute top-0 right-0 w-24 h-24 bg-brand-purple/5 rounded-full  group-hover:bg-brand-purple/10 transition" />
                    <span className="text-xs font-mono font-bold text-slate-500 uppercase tracking-widest block">Completion Chance</span>
                    <div className="relative flex items-center justify-center my-4 h-24 w-24">
                      ...
                      <span className="absolute font-sans font-black text-2xl text-slate-100">
                  ...
                  {}
                  <div className="md:col-span-2 bg-slate-900 border border-slate-800/50 rounded-xl p-5 flex flex-col justify-between shadow-md">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-mono font-bold text-slate-500 uppercase tracking-widest">Workload Risk Assessment</span>
                ...
                {}
                <div className="p-5 rounded-xl bg-slate-900 border border-slate-800/50 from-indigo-950/20 via-indigo-900/10 to-slate-900/50 border border-indigo-500/15 relative overflow-hidden group">
                  ...
                  <h3 className="font-semibold text-xs text-indigo-300 uppercase font-mono tracking-wider flex items-center gap-1.5">
                    <Award className="w-4 h-4 text-slate-300" /> Co-Pilot Motivational Mindset
                  </h3>
                ...
                {}
                <div className="bg-slate-900 border border-slate-800/50 rounded-xl p-5 shadow-sm">
                  <h3 className="font-semibold text-sm text-slate-100 border-b border-slate-800/50 pb-2.5 flex items-center gap-2">
                    <CheckCircle2 className="w-4.5 h-4.5 text-slate-300" /> Task Priority & Strategic Ordering
                  </h3>
                  <div className="mt-4 space-y-3">
                    {activePlan.priorities.map((item, idx) => (
                      <div 
                        key={idx}
                        className="p-3.5 rounded-xl bg-slate-950 border border-slate-800 flex items-start gap-3.5 hover:border-indigo-500/10 transition duration-200"
                      >
                        <div className="w-6 h-6 rounded-lg bg-brand-purple/10 border border-indigo-500/20 text-[11px] font-bold text-slate-300 flex items-center justify-center shrink-0 mt-0.5">
                          {item.rank}
                        </div>
                ...
                <div className="space-y-3">
                  <h3 className="font-semibold text-sm text-slate-100 flex items-center gap-2">
                    <CalendarDays className="w-4.5 h-4.5 text-slate-300" /> Personalized Study blocks
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {activePlan.schedule.map((block, idx) => (
                      <div 
                        key={idx}
                        className="p-4 rounded-xl bg-slate-900 border border-slate-800/50 flex flex-col justify-between hover:bg-slate-900 transition duration-200 shadow-md relative overflow-hidden"
                      >
                        <div>
                          <div className="flex items-center justify-between gap-2">
                            <span className="text-xs font-mono text-slate-300 font-semibold uppercase bg-brand-purple/10 px-2 py-0.5 rounded border border-indigo-500/10">
                              {block.time_block}
                            </span>
                          ...
                          <h4 className="font-semibold text-xs text-slate-100 mt-2.5 flex items-center gap-1.5">
                            <Brain className="w-3.5 h-3.5 text-slate-300 shrink-0" /> {block.focus_area}
                          </h4>
                          <p className="text-[11px] text-slate-300 mt-1.5 leading-relaxed">{block.details}</p>
                        </div>
                        <div className="border-t border-slate-800 mt-3 pt-2.5 flex flex-wrap gap-1">
                          {block.tasks.map((t, tIdx) => (
                            <span key={tIdx} className="text-[11px] font-mono text-slate-400 bg-slate-950 border border-slate-800 px-2 py-0.5 rounded-full">
                              {t}
                            </span>
                          ))}
                        </div>
                ...
                {}
                <div className="p-4 rounded-xl bg-slate-900 border border-slate-800/50 flex gap-4 items-start shadow-md">
              ...
              >
                <div className="w-14 h-14 rounded-xl bg-slate-900 border border-slate-800/50 flex items-center justify-center mb-4">
                  <Sparkles className="w-7 h-7 text-slate-300 " />
                </div>
                <h3 className="font-sans font-semibold text-base text-slate-100">No Active Strategic Plan</h3>
