    ...
    <div className="fixed bottom-24 right-6 z-50 font-sans" id="floating-voice-assistant">
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute bottom-16 right-0 w-[380px] h-[500px] max-w-[calc(100vw-3rem)] rounded-2xl bg-slate-950 border border-slate-800 shadow-2xl flex flex-col overflow-hidden backdrop-blur-xl"
          >
            {}
            <div className="px-4 py-3 bg-slate-900 border-b border-slate-800 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${isListening ? 'bg-rose-500/20 text-rose-400' : 'bg-indigo-600/10 text-indigo-400'}`}>
                  {isListening ? <Mic className="w-4.5 h-4.5 animate-pulse" /> : <Bot className="w-4.5 h-4.5" />}
                </div>
                <div>
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className={`p-2 rounded-lg transition ${voiceEnabled ? 'text-indigo-400 hover:bg-slate-800' : 'text-slate-500 hover:bg-slate-800'}`}
                >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin">
              ...
              {messages.map((m) => {
                ...
                return (
                  <div key={m.id} className={`flex gap-3 ${isAI ? "justify-start" : "justify-end"}`}>
                    {isAI && (
                      <div className="w-7 h-7 rounded-lg bg-slate-900 border border-slate-800 shrink-0 flex items-center justify-center">
              ...
              {isLoading && (
                <div className="flex gap-3 justify-start">
                  ...
                  <div className="p-3 bg-slate-900/60 border border-slate-800 rounded-2xl rounded-tl-none flex items-center gap-1.5">
              ...
              <form onSubmit={handleSubmit} className="flex gap-2 items-center">
                ...
                <div className="flex-1 relative">
                  <input
                    ...
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-4 pr-10 py-2.5 text-[13px] text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition font-sans"
                  />
                  {isListening && (
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-0.5">
                      <div className="w-1 h-3 bg-indigo-500 rounded-full animate-[bounce_1s_infinite_0ms]" />
                      <div className="w-1 h-4 bg-indigo-500 rounded-full animate-[bounce_1s_infinite_100ms]" />
                      <div className="w-1 h-2 bg-indigo-500 rounded-full animate-[bounce_1s_infinite_200ms]" />
                    </div>
                  )}
                </div>
                ...
                <button
                  ...
                  className="p-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 disabled:hover:bg-indigo-600 text-white transition-all active:scale-95 shrink-0"
                >
      ...
      <div className="relative">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className={`w-14 h-14 rounded-full flex items-center justify-center shadow-2xl border transition-colors duration-300 relative group ${
            isOpen 
              ? "bg-slate-900 border-slate-800 text-white" 
              : "bg-gradient-to-tr from-indigo-600 to-purple-600 border-indigo-400/50 text-white"
          }`}
        >
