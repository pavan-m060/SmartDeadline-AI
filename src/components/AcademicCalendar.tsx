...
export default function AcademicCalendar({ 
  ...
  const [_draggedItem, setDraggedItem] = useState<{ type: "assignment" | "milestone" | "session"; id: string; assignmentId?: string } | null>(null);
  ...
  const getPriorityStyle = (priority: Priority) => {
    switch (priority) {
      ...
      case "MEDIUM":
        return "bg-brand-purple/10 hover:bg-brand-purple/15 border-indigo-500/30 text-slate-300";
      default:
        return "bg-slate-800/50 hover:bg-slate-800/70 border-slate-800 text-slate-300";
    }
  };
  ...
  const handleScheduleSubmit = async (e: React.FormEvent) => {
    ...
    const newSessions: StudySession[] = datesList.map((d, index) => ({
      id: `session-scheduled-${Date.now()}-${Math.random().toString(36).substr(2, 9)}-${index}`,
      assignmentId,
  ...
  const handleExportICS = () => {
    const ics = [
      "BEGIN:VCALENDAR",
      "VERSION:2.0",
      "PRODID:-//Smart Deadline AI//Academic Calendar Exporter//EN",
      "CALSCALE:GREGORIAN",
      "METHOD:PUBLISH"
    ];
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6 text-slate-100 min-h-[calc(100vh-6rem)] relative" id="academic-calendar-wrapper">
      ...
      <div className="flex-1 flex flex-col gap-5 min-w-0">
        ...
        {}
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-slate-900 p-4 rounded-xl border border-slate-800 backdrop-">
          {}
          <div className="flex flex-wrap items-center gap-2">
            <button 
              ...
              className="p-2 hover:bg-slate-800 text-slate-400 hover:text-slate-100 rounded-xl border border-slate-800 transition cursor-pointer"
              title="Previous"
            >
              <ChevronLeft className="w-4 h-4" />
            </button>
            
            <h3 className="text-sm md:text-base font-bold text-slate-100 min-w-[140px] text-center font-sans tracking-tight bg-slate-950 px-3 py-1.5 rounded-xl border border-slate-900">
            ...
            <button
              onClick={handleToday}
              className="px-3 py-1.5 bg-slate-950 hover:bg-slate-900 text-slate-300 font-mono text-xs font-bold rounded-lg border border-slate-800 transition cursor-pointer font-medium"
            >
              TODAY
            </button>
          </div>
          ...
          {}
          <div className="flex items-center gap-1.5 flex-wrap bg-slate-950 p-1.5 rounded-xl border border-slate-900 text-[11px] font-medium">
            <span className="text-slate-500 font-mono text-[11px] font-medium px-2">Show:</span>
            <button
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <button
              ...
              className="p-2 bg-brand-purple hover:bg-brand-purple-dark shadow-sm text-slate-100 rounded-xl flex items-center justify-center gap-1 text-xs font-bold transition shadow-sm cursor-pointer"
            ...
            <button
              ...
              className="p-2 bg-slate-800 hover:bg-slate-750 border border-slate-700 text-slate-300 hover:text-slate-100 rounded-xl flex items-center justify-center gap-1 text-xs font-bold transition cursor-pointer"
        ...
        <AnimatePresence mode="wait">
          ...
          {viewMode === "month" && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="bg-slate-900 border border-slate-800/50 rounded-xl overflow-hidden shadow-sm"
            >
              {}
              <div className="grid grid-cols-7 border-b border-slate-800/50 bg-slate-900">
                {DAYS_SHORT.map((day, idx) => (
                  <div key={idx} className="py-2.5 text-center text-xs font-mono font-bold tracking-widest text-slate-400 border-r border-slate-800/50 last:border-r-0">
                    {day.toUpperCase()}
                  </div>
                ))}
              </div>
                ...
                {monthDays.map(({ date, isCurrentMonth }, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`min-h-[120px] p-2 border-b border-r border-slate-800/50 flex flex-col justify-between transition relative group cursor-pointer ${
                        isCurrentMonth ? "bg-slate-950" : "bg-slate-950 text-slate-600 opacity-60"
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between">
                        <span className={`text-[11px] font-mono font-bold w-6 h-6 flex items-center justify-center rounded-full ${
                          isDayToday 
                            ? "bg-indigo-600 text-slate-100 font-black scale-105 shadow-md" 
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition">
                          <button
                            ...
                            className="p-1 hover:bg-indigo-600 hover:text-slate-100 bg-slate-950 border border-slate-800 rounded text-[11px] font-bold text-slate-400 flex items-center transition"
                        ...
                        {visibleExams.map(exam => {
                          ...
                          return (
                            <div
                              ...
                              className={`px-1.5 py-0.5 rounded-md border text-xs font-semibold leading-tight truncate flex items-center gap-1 transition shadow-sm cursor-grab active:cursor-grabbing ${
                                isTaskOverdue 
                                  ? "border-rose-500 bg-rose-500/20 text-rose-300 hover:bg-rose-500/25 " 
                        ...
                        {visibleAssignments.map(a => {
                          ...
                          return (
                            <div
                              ...
                              className={`px-1.5 py-0.5 rounded-md border text-xs font-medium leading-tight truncate flex items-center gap-1 transition shadow-sm cursor-grab active:cursor-grabbing ${
                                isCompleted 
                                  ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400/80 line-through" 
                                  : isTaskOverdue 
                                    ? "bg-rose-500/20 border-rose-500 text-rose-300 hover:bg-rose-500/25 "
                                    : style
                              }`}
                        ...
                        {visibleMilestones.map(({ assignment, milestone }) => {
                          ...
                          return (
                            <div
                              ...
                              className={`px-1.5 py-0.5 rounded-md border text-xs leading-tight flex items-center justify-between gap-1 transition shadow-sm cursor-grab active:cursor-grabbing ${
                                milestone.completed 
                        ...
                        {visibleSessions.map((session, sIdx) => {
                          ...
                          return (
                            <div
                              ...
                              className="px-1.5 py-0.5 rounded-md border border-sky-500/30 bg-sky-500/5 text-sky-300 text-xs leading-tight truncate flex items-center gap-1 transition shadow-sm cursor-grab active:cursor-grabbing hover:border-sky-400/50"
                      ...
                      {}
                      {totalItems > 4 && (
                        <span className="text-[8px] font-mono font-bold text-slate-300 text-right block pr-1">
                          +{totalItems - 4} MORE
                        </span>
                      )}
                    </div>
              ...
              {weekDays.map((date, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-slate-900 border border-slate-800/50 rounded-xl p-4 flex flex-col min-h-[400px] transition cursor-pointer relative ${
                      isDayToday ? "border-2 border-indigo-500 bg-indigo-950/15 shadow-sm" : ""
                    } ${
                      isDaySelected ? "ring-2 ring-indigo-500 bg-indigo-950/5 shadow-sm" : ""
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between border-b border-slate-800/50 pb-3 mb-4">
                      <div>
                        <div className="flex items-center gap-1.5">
                          <span className="text-xs font-mono font-bold font-medium text-slate-500">
                            {DAYS_SHORT[date.getDay()]}
                          </span>
                          {isDayToday && (
                            <span className="px-1.5 py-0.5 bg-brand-purple/20 text-slate-300 font-mono text-[8px] font-bold rounded uppercase ">
                              Today
                            </span>
                          )}
                        </div>
                    ...
                    <div className="flex-1 space-y-2.5 overflow-y-auto max-h-[380px] scrollbar-thin">
                      {visibleExams.length === 0 && 
                       ...
                       visibleMilestones.length === 0 && (
                        <div className="h-full flex flex-col items-center justify-center text-center p-4 py-12">
                          ...
                          <span className="text-xs font-mono text-slate-600 uppercase tracking-widest">Clear Agenda</span>
                        </div>
                      )}
                        ...
                        return (
                          <div
                            ...
                            className={`p-3 rounded-xl flex flex-col gap-1.5 text-xs transition cursor-grab active:cursor-grabbing shadow-inner border-2 ${
                              isTaskOverdue
                                ? "bg-rose-500/20 border-rose-500 text-rose-200 hover:border-rose-400 "
                                : "bg-amber-500/10 border-amber-500/40 hover:border-amber-400 text-amber-200"
                            }`}
                          >
                            <div className="flex items-center justify-between">
                              <span className={`text-[11px] border px-1.5 py-0.5 rounded font-mono font-bold ${
                                isTaskOverdue ? "bg-rose-500/25 border-rose-500/40 text-rose-300" : "bg-amber-500/25 border-amber-500/40 text-amber-300"
                              }`}>
                                {exam.course}
                              </span>
                              <span className={`text-[11px] font-mono font-bold flex items-center gap-1 ${isTaskOverdue ? "text-rose-400" : "text-amber-450"}`}>
                                {isTaskOverdue ? (
                                  <>
                                  ...
                                  <>
                                    <Award className="w-3 h-3 text-amber-400 " /> EXAM
                                  </>
                                )}
                              </span>
                            </div>
                            <h5 className="font-bold text-slate-100 leading-snug line-clamp-2">
                              {exam.title}
                            </h5>
                          </div>
                        );
                          ...
                          <div
                            ...
                            className={`p-3 rounded-xl border flex flex-col justify-between gap-2 text-xs transition cursor-grab active:cursor-grabbing ${
                              isCompleted 
                                ...
                                : isTaskOverdue
                                  ? "bg-rose-500/20 border-rose-500 text-rose-300 hover:border-rose-400 "
                                  : style
                            }`}
                          >
                            <div>
                              <div className="flex items-center justify-between gap-1 mb-1">
                                <span className="text-[11px] text-xs font-medium text-slate-500 px-1.5 py-0.5 rounded-md bg-slate-950 text-slate-300">
                                  {a.course}
                                </span>
                              ...
                              <h5 className={`font-bold text-slate-100 leading-snug line-clamp-2 ${isCompleted ? "line-through text-slate-400" : ""}`}>
                                {a.title}
                              </h5>
                            </div>
                            <div className="flex items-center justify-between text-[11px] font-mono border-t border-slate-800 pt-1.5 mt-1 text-slate-400">
                              ...
                              {a.weight && <span className="text-slate-300">{a.weight}% Grade</span>}
                            </div>
                          </div>
                        );
                      })}
                      ...
                      {visibleMilestones.map(({ assignment, milestone }) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span className={`text-[11px] font-mono border px-1.5 py-0.5 rounded font-bold ${
                      ...
                      {visibleSessions.map((session, sIdx) => {
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span className="text-[11px] bg-sky-500/10 border border-sky-500/20 px-1.5 py-0.5 rounded-md font-mono font-bold flex items-center gap-1">
                                <Clock className="w-2.5 h-2.5" /> STUDY SESSION
                              </span>
                              <span className="text-xs font-mono font-bold text-sky-300">{session.durationMinutes} min</span>
                            </div>
                            <p className="text-[11px] text-slate-100 font-medium mt-1">
                              {related ? related.title : "Independent Focus"}
                            </p>
                            {session.notes && (
                              <p className="text-xs text-sky-300/75 italic line-clamp-2">
                                "{session.notes}"
                              </p>
                            )}
                          </div>
                    ...
                    <button
                      ...
                      className="mt-3 py-1.5 w-full bg-slate-950 hover:bg-indigo-600 text-xs font-mono font-bold text-slate-400 hover:text-slate-100 rounded-lg border border-slate-800 flex items-center justify-center gap-1 transition"
                    >
          ...
          {viewMode === "day" && (
            ...
            >
              {}
              <div className="lg:col-span-2 bg-slate-900 border border-slate-800/50 rounded-xl p-6 space-y-6">
                <div className="flex items-center justify-between border-b border-slate-800/50 pb-4">
                  <div>
                    ...
                    <h4 className="text-xl font-bold font-sans text-slate-100">
                      {MONTHS[selectedDate.getMonth()]} {selectedDate.getDate()}, {selectedDate.getFullYear()}
                    </h4>
                  </div>
                  
                  <div className="text-right">
                    <span className="text-xs text-slate-500 block uppercase font-mono font-bold">Planned Focus</span>
                ...
                <div className="relative pl-12 border-l-2 border-slate-800 space-y-8 py-4">
                  
                  {}
                  <div className="relative">
                    <div className="absolute -left-[54px] w-4 h-4 rounded-full border-4 border-slate-950 bg-brand-purple shadow-md" />
                    <div className="space-y-3">
                      <span className="text-xs font-mono font-bold text-slate-500 tracking-wider uppercase">ALL DAY DELIVERABLES & EXAMS</span>
                      ...
                      {(() => {
                        ...
                        if (total === 0) {
                          return (
                            <p className="text-xs text-slate-500 italic p-3.5 bg-slate-950 border border-slate-900 rounded-xl">
                              ...
                              return (
                                <div
                                  ...
                                  className={`p-4 rounded-xl flex flex-col justify-between gap-3 shadow border-2 transition hover:scale-[1.01] cursor-pointer ${
                                    isTaskOverdue
                                      ? "bg-rose-500/20 border-rose-500 text-rose-200  animate-duration-1000"
                                  ...
                                  <div>
                                    <div className="flex items-center justify-between gap-1 mb-1">
                                      <span className={`text-[11px] font-mono font-bold px-1.5 py-0.5 rounded-md ${
                                        isTaskOverdue ? "bg-rose-500/25 border-rose-500/40 text-rose-300" : "bg-slate-950 text-amber-300"
                                      }`}>
                                        {exam.course}
                                      </span>
                                      <span className={`text-[11px] font-mono font-bold flex items-center gap-1 ${isTaskOverdue ? "text-rose-400 " : "text-amber-400"}`}>
                                        {isTaskOverdue ? (
                                          <>
                                    ...
                                    <h5 className="font-bold text-slate-100 text-sm">{exam.title}</h5>
                                  </div>
                                  <div className="border-t border-slate-800 pt-2 flex items-center justify-between">
                                    <span className={`text-xs font-mono ${isTaskOverdue ? "text-rose-450 font-bold" : "text-amber-400"}`}>Weight: {exam.weight}%</span>
                                    {onSelectAssignmentForTimer && (
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                        ...
                                        className={`px-2 py-1 text-xs font-semibold rounded-lg font-mono transition cursor-pointer ${
                                ...
                                <div
                                  ...
                                  className={`p-4 rounded-xl border flex flex-col justify-between gap-3 shadow transition hover:scale-[1.01] cursor-pointer ${
                                    isCompleted 
                                      ...
                                      : isTaskOverdue
                                        ? "bg-rose-500/20 border-rose-500 text-rose-300  animate-duration-1000"
                                        : style
                                  }`}
                                >
                                  <div>
                                    <div className="flex items-center justify-between gap-1 mb-1">
                                      <span className="text-[11px] font-mono font-bold px-1.5 py-0.5 rounded-md bg-slate-950 text-slate-300">
                                        {a.course}
                                      </span>
                                    ...
                                    <h5 className={`font-bold text-sm text-slate-100 ${isCompleted ? "line-through text-slate-400" : ""}`}>{a.title}</h5>
                                  </div>
                                  <div className="border-t border-slate-800 pt-2 flex items-center justify-between text-xs">
                                    ...
                                    {onSelectAssignmentForTimer && !isCompleted && (
                                      <button
                                        ...
                                        className="px-2 py-1 bg-indigo-600/20 hover:bg-indigo-600 text-xs font-semibold rounded-lg text-indigo-300 hover:text-slate-100 font-mono transition cursor-pointer"
                                      >
                                        Start Focus
                                      </button>
                                    )}
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-3">
                      <span className="text-xs font-mono font-bold text-slate-500 tracking-wider uppercase">ACTIVE SYLLABUS MILESTONES</span>
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center gap-3">
                                    ...
                                    />
                                    <div>
                                      <span className={`text-[11px] font-mono px-1.5 py-0.5 rounded font-bold uppercase ${
                                      ...
                                      <p className="text-xs text-slate-400">Parent Task: {assignment.title}</p>
                                    </div>
                                  </div>
                                </div>
                              );
                  ...
                  <div className="relative">
                    ...
                    <div className="space-y-3">
                      <span className="text-xs font-mono font-bold text-slate-500 tracking-wider uppercase">SCHEDULED STUDY LOGS & FOCUS SESSIONS</span>
                              ...
                              return (
                                ...
                                >
                                  <div className="space-y-1.5">
                                    <span className="text-[11px] bg-sky-500/15 border border-sky-500/20 px-2 py-0.5 rounded-full font-mono font-bold text-sky-300 uppercase">
                                  ...
                                  <div className="flex items-center gap-4">
                                    <div className="text-right">
                                      <span className="text-lg font-black text-slate-100 font-mono block">{session.durationMinutes}m</span>
                                      <span className="text-[11px] font-mono text-sky-400 font-medium block font-bold">Focus Target</span>
                                    </div>
              ...
              {}
              <div className="bg-slate-900 border border-slate-800/50 rounded-xl p-6 space-y-6">
                <h4 className="text-sm font-bold text-slate-100 font-sans flex items-center gap-2">
                  <Target className="w-4 h-4 text-slate-300" /> Day Summary Checklist
                </h4>
                    ...
                    <div className="space-y-6">
                      <div className="bg-slate-950 p-4 rounded-xl border border-slate-900 space-y-2">
                        <div className="flex justify-between items-center text-xs font-mono">
                          ...
                          <span className="text-slate-100 font-bold">{completed} / {total} Completed</span>
                        </div>
                      ...
                      <div className="space-y-3">
                        <span className="text-xs font-mono font-bold font-medium text-slate-500">Day Tasks</span>
                        {dayAssignments.length === 0 ? (
                          <p className="text-xs text-slate-500 italic text-center py-4">No tasks due today.</p>
                        ) : (
                          dayAssignments.map(task => (
                            <div key={task.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-950 border border-slate-900">
                              ...
                              <span className="text-[11px] font-mono text-slate-300 px-1.5 py-0.5 rounded bg-brand-purple/10 border border-indigo-500/15 shrink-0">
                                {task.course}
                              </span>
                            </div>
                          ))
      ...
      <AnimatePresence>
        {showUnscheduledPanel && (
          <motion.div
            ...
            exit={{ opacity: 0, x: 25 }}
            className="w-full lg:w-3/12 xl:w-80 bg-slate-900 border border-slate-800/50 rounded-xl p-4 flex flex-col h-[calc(100vh-6rem)] overflow-hidden shadow-sm backdrop- shrink-0"
          >
            <div className="flex items-center justify-between border-b border-slate-800/50 pb-3 mb-4 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="font-sans font-bold text-xs text-slate-100">Unscheduled Milestones</h4>
                  <p className="text-[11px] text-slate-400 font-mono">Drag on day to schedule</p>
                </div>
              </div>
              <button 
                onClick={() => setShowUnscheduledPanel(false)}
                className="p-1 hover:bg-slate-800 rounded-lg text-slate-500 hover:text-slate-100 cursor-pointer"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            {}
            <div className="p-3 rounded-xl bg-purple-950/20 border border-purple-500/10 text-xs text-purple-300 leading-relaxed mb-4 flex gap-2 items-start">
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 scrollbar-thin">
              {unscheduledMilestones.length === 0 ? (
                <div className="text-center py-12 text-slate-500 space-y-2">
                  ...
                  <p className="text-xs leading-relaxed max-w-[180px] mx-auto">Generate a master study plan in the AI Planner to load milestone sequences.</p>
                </div>
              ) : (
                unscheduledMilestones.map(({ assignment, milestone }) => (
                  <div
                    ...
                    draggable
                    onDragStart={(e) => handleDragStart(e, "milestone", milestone.id, assignment.id)}
                    className="p-3 bg-slate-950 border border-slate-850 hover:border-purple-500/30 rounded-xl cursor-grab active:cursor-grabbing transition duration-200 hover:bg-purple-950/5 group flex flex-col gap-1 shadow"
                  ...
                  >
                    ...
                    <p className="text-[11px] text-slate-500 truncate">For: {assignment.title}</p>
                  </div>
                ))
              )}
            </div>
      ...
      <AnimatePresence>
        {isScheduleModalOpen && (
          <div className="fixed inset-0 bg-slate-950 backdrop- flex items-center justify-center z-50 p-4">
            <motion.div
            ...
            >
              <div className="p-4 border-b border-slate-800/50 flex items-center justify-between bg-slate-950">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-sans font-bold text-sm text-slate-100">Schedule Study Session</h3>
                </div>
                <button
                  onClick={() => setIsScheduleModalOpen(false)}
                  className="p-1 hover:bg-slate-800 rounded-lg text-slate-400 hover:text-slate-100 cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              <form onSubmit={handleScheduleSubmit} className="p-5 space-y-4 text-slate-200">
                ...
                {}
                <div className="space-y-1.5">
                  <label className="text-xs font-mono font-bold font-medium text-slate-400">Associated Course/Task</label>
                  <select
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <label className="text-xs font-mono font-bold font-medium text-slate-400">Target Date</label>
                    <input
                      type="date"
                  ...
                  <div className="space-y-1.5">
                    <label className="text-xs font-mono font-bold font-medium text-slate-400">Start Time</label>
                    <input
                      type="time"
                ...
                {}
                <div className="space-y-1.5">
                  <label className="text-xs font-mono font-bold font-medium text-slate-400">Block Duration</label>
                  <select
                ...
                <div className="space-y-1.5 bg-slate-950 p-3 rounded-xl border border-slate-800">
                  <label className="text-xs font-mono font-bold font-medium text-slate-400 block">Recurrence Rule</label>
                  <select
                    value={scheduleForm.recurrence}
                    onChange={(e) => setScheduleForm(prev => ({ ...prev, recurrence: e.target.value }))}
                    className="w-full mt-1.5 px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-100 focus:outline-none focus:border-indigo-500"
                    required
                  >
                  ...
                  {scheduleForm.recurrence !== "none" && (
                    <div className="mt-3 space-y-1">
                      <label className="text-xs font-mono text-slate-400 block">Number of Occurrences (Max 12)</label>
                      <input
                        type="number"
                        min={1}
                        max={12}
                ...
                {}
                <div className="space-y-1.5">
                  <label className="text-xs font-mono font-bold font-medium text-slate-400">Focus Goal / Notes</label>
                  <input
                    type="text"
                ...
                <div className="flex gap-2.5 pt-2">
                  <button
                    type="button"
                    onClick={() => setIsScheduleModalOpen(false)}
                    className="flex-1 py-2 bg-slate-800 hover:bg-slate-750 text-slate-300 hover:text-slate-100 rounded-xl text-xs font-bold transition cursor-pointer"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-2 bg-brand-purple hover:bg-brand-purple-dark shadow-sm text-slate-100 rounded-xl text-xs font-bold transition shadow-sm shadow-indigo-600/20 cursor-pointer"
                  >
                    Schedule Focus
                  </button>
                </div>
      ...
      <AnimatePresence>
        {selectedEventDetails && (
          <div className="fixed inset-0 bg-slate-950 backdrop- flex items-center justify-center z-50 p-4" id="event-details-modal">
            <motion.div
              initial={{ opacity: 0, scale: 0.95 }}
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="bg-slate-900 border border-slate-800/50 rounded-xl w-full max-w-lg overflow-hidden shadow-sm border-slate-800 flex flex-col max-h-[90vh]"
            >
              {}
              <div className="p-4 border-b border-slate-800/50 flex items-center justify-between bg-slate-950">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-sans font-bold text-sm text-slate-100 capitalize">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs font-mono font-bold font-medium px-2 py-0.5 rounded bg-brand-purple/10 border border-indigo-500/15 text-indigo-300">
                            {assignment.course}
                          </span>
                          <div className="flex gap-1.5">
                            {isTaskOverdue && (
                              <span className="text-[11px] font-mono font-bold text-rose-400 bg-rose-950/40 px-2 py-0.5 rounded border border-rose-500/20 flex items-center gap-1 ">
                        ...
                        <h4 className="text-lg font-bold text-slate-100 font-sans leading-snug mt-1">
                          {assignment.title}
                        </h4>
                      </div>

                      {}
                      <div className="grid grid-cols-2 gap-4 bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs font-medium">
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Due Date</span>
                        ...
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Course Weight</span>
                          <span className="text-slate-100">
                            {assignment.weight ? `${assignment.weight}% of Final Grade` : "N/A"}
                          </span>
                        </div>
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Estimated Hours</span>
                          <span className="text-slate-100 flex items-center gap-1.5">
                        ...
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Difficulty</span>
                      ...
                      {assignment.description && (
                        <div className="space-y-1.5">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Description</span>
                          <p className="text-sm text-slate-300 leading-relaxed bg-slate-950 p-3.5 rounded-xl border border-slate-800 max-h-[120px] overflow-y-auto scrollbar-thin">
                            {assignment.description}
                          </p>
                        </div>
                      )}
                      ...
                      {assignment.milestones && assignment.milestones.length > 0 && (
                        <div className="space-y-2.5">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Syllabus Milestones</span>
                          <div className="space-y-2 max-h-[140px] overflow-y-auto scrollbar-thin">
                            {assignment.milestones.map(m => (
                              <div key={m.id} className="flex items-center justify-between p-2.5 rounded-xl bg-slate-950 border border-slate-900">
                                <div className="flex items-center gap-2 min-w-0">
                                  <input
                                    type="checkbox"
                                    checked={m.completed}
                      ...
                      <div className="flex gap-2.5 pt-4 border-t border-slate-850">
                        <button
                          ...
                          className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer ${
                            isCompleted 
                              ? "bg-slate-800 hover:bg-slate-750 text-slate-300 hover:text-slate-100 border border-slate-750" 
                              : "bg-emerald-600 hover:bg-emerald-500 text-slate-100 shadow-sm shadow-emerald-600/10"
                          }`}
                        >
                          {isCompleted ? (
                            <>Reopen Deliverable</>
                        ...
                        {onSelectAssignmentForTimer && !isCompleted && (
                          <button
                            ...
                            className="flex-1 py-2.5 bg-brand-purple hover:bg-brand-purple-dark shadow-sm text-slate-100 rounded-xl text-xs font-bold transition shadow-sm shadow-indigo-600/20 flex items-center justify-center gap-2 cursor-pointer"
                          >
                ...
                {selectedEventDetails.type === "session" && (() => {
                  ...
                  return (
                    <div className="space-y-4">
                      {}
                      <div className="space-y-1">
                        <span className="text-xs font-mono font-bold font-medium px-2 py-0.5 rounded bg-sky-500/10 border border-sky-500/15 text-sky-300">
                          Focus Block
                        </span>
                        <h4 className="text-lg font-bold text-slate-100 font-sans mt-1">
                          {related ? related.title : "Independent Study Focus"}
                        </h4>
                        {related && (
                          <p className="text-xs text-slate-400">Course: <span className="text-slate-300 font-semibold">{related.course}</span></p>
                        )}
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs font-medium">
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Session Date</span>
                          <span className="text-slate-100 flex items-center gap-1.5">
                        ...
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Start Time</span>
                          <span className="text-slate-100 flex items-center gap-1.5">
                        ...
                        <div className="space-y-1 col-span-2">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Duration</span>
                          <span className="text-slate-100 text-sm font-bold flex items-center gap-1.5">
                      ...
                      {session.notes && (
                        <div className="space-y-1.5">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Session Goals / Notes</span>
                      ...
                      <div className="flex gap-2.5 pt-4 border-t border-slate-850">
                        {onDeleteStudySession && (
                          <button
                            ...
                            className="flex-1 py-2.5 bg-rose-950/30 hover:bg-rose-600 border border-rose-500/20 hover:border-rose-500 text-rose-300 hover:text-slate-100 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer"
                          >
                ...
                {selectedEventDetails.type === "milestone" && (() => {
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-xs font-mono font-bold font-medium px-2 py-0.5 rounded bg-purple-500/10 border border-purple-500/15 text-purple-300">
                            Syllabus Milestone
                          </span>
                          {isMOverdue && (
                            <span className="text-[11px] font-mono font-bold text-rose-400 bg-rose-950/40 px-2 py-0.5 rounded border border-rose-500/20 flex items-center gap-1 ">
                        ...
                        <h4 className="text-lg font-bold text-slate-100 font-sans mt-1 leading-snug">
                          {milestone.title}
                        </h4>
                        <p className="text-xs text-slate-400">
                          For Task: <span className="text-slate-300 font-semibold">{parentAssignment.title}</span> ({parentAssignment.course})
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4 bg-slate-950 p-4 rounded-xl border border-slate-800 text-xs font-medium">
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Scheduled Date</span>
                          <span className="text-slate-100 flex items-center gap-1.5">
                        ...
                        <div className="space-y-1">
                          <span className="text-slate-500 text-xs font-mono font-medium block">Milestone Status</span>
                      ...
                      <div className="flex gap-2.5 pt-4 border-t border-slate-850">
                        <button
                          ...
                          className={`flex-1 py-2.5 rounded-xl text-xs font-bold transition flex items-center justify-center gap-2 cursor-pointer ${
                            milestone.completed 
                              ? "bg-slate-800 hover:bg-slate-750 text-slate-300 hover:text-slate-100 border border-slate-750" 
                              : "bg-purple-600 hover:bg-purple-500 text-slate-100 shadow-sm shadow-purple-600/10"
                          }`}
                        >
                          {milestone.completed ? (
                            <>Reopen Milestone</>
