...
export default function ProcrastinationBuster({ assignments, onStartStartingSprint }: ProcrastinationBusterProps) {
  ...
  const handleSeekSupport = async () => {
    ...
    try {
      ...
      setError(err.message || "Failed to contact SmartDeadline AI. Please verify your connection.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      {}
      <div>
        <h2 className="font-display font-bold text-3xl text-white tracking-tight">AI Procrastination Buster</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
        {}
        <div className="md:col-span-2 space-y-4 p-5 rounded-xl bg-slate-900 border border-slate-800 self-start">
          ...
          <div className="space-y-1.5">
            <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-wider">Assignment Under Friction</label>
            <select
          ...
          {}
          <div className="space-y-1.5">
            <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-wider">Core Psychological Barrier</label>
            <div className="space-y-1.5">
          ...
          {}
          <div className="space-y-1.5">
            <label className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-wider">Current Emotional State</label>
          ...
          <button
            ...
            className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-lg text-xs flex items-center justify-center gap-2 transition disabled:opacity-40 cursor-pointer"
          >
            {loading ? (
              <>
        ...
        <div className="md:col-span-3 flex flex-col justify-stretch">
          {nudge ? (
            ...
            <motion.div 
              ...
              className="p-6 rounded-xl bg-slate-900 border border-slate-800 space-y-6 flex flex-col justify-between h-full"
            >
              <div className="space-y-5">
                ...
                <div className="flex items-center gap-2.5 pb-4 border-b border-slate-800">
                  ...
                  <div>
                    <h4 className="text-[10px] font-mono font-bold text-slate-500 uppercase tracking-widest">Cognitive Pep Talk</h4>
                    <h3 className="text-sm font-semibold text-white">AI Intervention Active</h3>
                  </div>
                </div>

                {}
                <div className="space-y-1.5">
                  <h5 className="text-[11px] font-mono font-bold text-slate-400 uppercase tracking-wider">CBT Cognitive Reframing</h5>
                  <p className="text-slate-300 text-xs leading-relaxed">{nudge.explanation}</p>
                </div>

                {}
                <div className="p-3 bg-indigo-500/5 rounded-lg border border-indigo-500/10 italic text-indigo-300 text-xs">
                  "{nudge.encouragement}"
                </div>

                {}
                <div className="space-y-3">
                  <h5 className="text-[11px] font-mono font-bold text-slate-400 uppercase tracking-wider">Your 5-Minute Kickstarters</h5>
              ...
              <div className="mt-6 pt-5 border-t border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3">
                <div className="text-center sm:text-left">
                  <div className="text-xs font-semibold text-white">Target Milestone: {nudge.milestoneTitle}</div>
                  <div className="text-[10px] text-slate-500">Initiate a customized 10-minute starter sprint to beat resistance.</div>
                </div>
                <button
                  onClick={handleLaunchSprint}
                  className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-semibold shadow transition duration-200 cursor-pointer"
                >
            ...
            <div className="h-full border border-slate-800 border-dashed rounded-xl bg-slate-900/30 flex flex-col items-center justify-center p-8 text-center min-h-[350px]">
              <Smile className="w-10 h-10 text-slate-600 mb-3 animate-bounce" />
              <h4 className="text-sm font-semibold text-slate-300">Awaiting Diagnostics</h4>
              <p className="text-xs text-slate-500 max-w-xs mt-1">
                Select your procrastinated assignment and pinpoint your current roadblock in the left panel to request emotional/practical coaching from SmartDeadline AI.
              </p>
            </div>
          )}
        </div>
